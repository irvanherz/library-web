...
function EditBook(props){
	...
	const handleEditBook = () => {
		const data = bookData
		Axios.put(`${process.env.REACT_APP_API_HOST}/books/${props.data.id}`, data).then(
			result => {
				if (result.status === 200) {
					toast(
						{
							title: 'Success',
							description: 'Book succesfully edited.',
							time: 2000,
							type: 'success',
						}
					)
	...
	return props.trigger ? (
		<Modal trigger={props.trigger} onOpen={() => setBookData({})} >
			...
			<Modal.Content image>
				<Modal.Description>
					<Form>
						<Form.Field>
							<label>Title</label>
							<input placeholder='Title' defaultValue={props.data.title} value={bookData.title} onChange={e => handleChange('title', e.target.value)} />
						</Form.Field>
						<Form.Field>
							<label>Year</label>
							<Dropdown
						...
						<Form.Field>
							...
							<Dropdown
								...
								search
								allowAdditions
								defaultValue={props.data.authorId}
						...
						<Form.Field>
							...
							<Dropdown
								...
								search
								allowAdditions
								defaultValue={props.data.publisherId}
						...
						<Button type='submit' onClick={() => handleEditBook()}>Submit</Button>
					</Form>
				</Modal.Description>
			</Modal.Content>
		</Modal>
